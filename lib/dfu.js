    ...
    class DFU extends BLEDevice {
        ...
        setDfuMode() {
            return new Promise((resolve, reject) => {
                ...
                Promise.resolve()
                    .then(() => {
                        const auxAddress = this.device.address.substr(this.device.address.length - 5).replace(':', '-');
                        this.dfuName = `DFU-${auxAddress}`;

                        const dfuNameBuffer = new ArrayBuffer(this.dfuName.length);
                        const bufView = new Uint8Array(dfuNameBuffer);
